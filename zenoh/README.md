...
The model adapter renders the navigation question with the chat template loaded from each checkpoint and lets the checkpoint's EOS token IDs terminate generation. This supports both the Llama 3 8-frame and Qwen2 64-frame checkpoints without applying Llama-specific separator text to Qwen2. It requests deterministic greedy decoding with neutral sampling values, including `top_k`, while preserving model-specific token IDs and settings such as repetition penalty.

During model loading and generation the adapter also suppresses only exact, known advisories from the pinned Jetson dependency stack: the Hugging Face `resume_download` deprecation, PyTorch's Orin CC 8.7 packaging advisory, the intentional bitsandbytes CUDA 13.0 binary override, bitsandbytes' internal PyTorch deprecation, and the tokenizer special-token notice. Other warnings remain visible. This keeps runtime output actionable without hiding new compatibility failures.
